<?php
    ...
    if($service_avi_level1!=0){
        ...
        try {
            ...
            if ($response -> HighestSeverity != 'FAILURE' && $response -> HighestSeverity != 'ERROR'){
                ...
                $servicetype=($service_avi_level1==1) ? 'Regular' : 'Extra Changes';
                $product_type="Standard Overnight";
                
                try{
                    $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $pass);
                    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    $stmt1 = $conn->prepare("INSERT INTO `transit_time` (`vendor_id`, `vendor_name`, `product_type`, `sub_product`, `pickup_date`, `pickup_Time`, `from_pin`, `to_pin`, `ExpectedDateDelivery`, `ExpectedDatePOD`, `TAT`, `AdditionalDays`,`ServiceType`,`added_date`,`added_by`) VALUES ('2', 'FedEx', '$product_type', '$sub_product', '$pickup_date1', '$pickup_time', '$from_pin', '$to_pin', '$ExpectedDateDelivery', '$ExpectedDatePOD', '$days', '$AdditionalDays','$servicetype','$nowtime','$userid');");
                    $stmt1->execute();
    ...
    if($service_avi_level2!=0){
        ...
        $request['RequestedShipment']['ServiceType'] = $product_type2;
        ...
        try {
            ...
            if ($response -> HighestSeverity != 'FAILURE' && $response -> HighestSeverity != 'ERROR'){
                ...
                $months = floor(($diff - $years * 365*60*60*24) / (30*60*60*24));
                $days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
                $servicetype=($service_avi_level2==1) ? 'Regular' : 'Extra Changes';
                $product_type="Priority Overnight";
