    ...
    <body class="demo">
        ...
        <?php
            ...
            if (isset($_POST["from_pin"]) && !empty($_POST["from_pin"]) && isset($_POST["to_pin"]) && !empty($_POST["to_pin"])) {
                ...
                $package_details=array($weight,$length,$breath,$height,$purpose,$cost);
                ...
                try{
                    ...
                    if($couriervendor=="FedEx"){
                        ...
                        elseif($service=="STANDARD_OVERNIGHT" &&  $service_avi_level2!=0){
                            ...
                            $stmt5 = $conn->prepare("INSERT INTO `AirwayBill`(`ShipperName`, `ReceiverName`, `COD`, `PackageCount`, `ReferenceID`, `AWB_Date`, `CourierVendor`, `CourierService`, `AWB_Status`, `AWB_Link`,`CreatedByCompID`) VALUES ('$sender_info[1]','$receiver_info[1]','$COD',$packagecount,'$uid','$shipment_date','FedEx','$service','No Service','','$CompID')");
                            $stmt5->execute();
        ...
        <script>
            PDFObject.embed('<?php echo "./".$filepath;?>', "#pdf");
            document.getElementById("AWB_Number").value = "<?php echo $token; ?>";
            document.getElementById("info").style.display='block';
            var UID = '<?php echo $AWB_UID; ?>';
            var userid='<?php echo $userid; ?>';
            document.getElementById('myUniqueLinkId').href = 'download.php?UID='+UID+'&userid='+userid+'&action=DownloadAWB';
            </script>
        ...
        elseif($couriervendor=="BlueDart"){
            ...
            elseif($service=="eTailPrePaidGround" && $service_avi_level10!=0){
                ...
                $stmt5 = $conn->prepare("INSERT INTO `AirwayBill`(`ShipperName`, `ReceiverName`, `COD`, `PackageCount`, `ReferenceID`, `AWB_Date`, `CourierVendor`, `CourierService`, `AWB_Status`, `AWB_Link`,`CreatedByCompID`) VALUES ('$sender_info[1]','$receiver_info[1]','$COD',$packagecount,'$uid','$shipment_date','BlueDart','$service','No Service','','$CompID')");
                $stmt5->execute();
