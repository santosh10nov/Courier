...
<html lang="en">
<head>
<title>rShipper Service- Receive Courier List</title>
<meta charset="utf-8">
...
<?php echo  $reg_user->Navigation(); ?>

<div class="container" id="container">
<h1 align="center">Receive Courier List</h1>
<!-- form start -->
...
<td>
<input type="text" class="form-control" placeholder="Sender Name" id="receiver_name">
</td>
<td>
...
function list(){
    ...
    xmlhttp.open("GET","receive_ajax.php?action=select&row_count="+row_count+"&id="+id,true);
    xmlhttp.send();
...
function pagecount(i){
    ...
    xmlhttp.open("GET","receive_ajax.php?action=pagecount&row_count="+row_count+"&id="+id,true);
    xmlhttp.send();
...
function changePagination(list_id){
    ...
    xmlhttp.open("GET","receive_ajax.php?action=changePagination&page="+page+"&row_count="+row_count+"&id="+id,true);
    xmlhttp.send();
...
function Search(){
    ...
    if(receiver_name=="" && shipment_date=="" && vendor=="" && airwaybill_number=="" ){
        ...
        xmlhttp.open("GET","receive_ajax.php?action=search&receiver_name="+receiver_name+"&shipment_date="+shipment_date+"&vendor="+vendor+"&airwaybill_number="+airwaybill_number+"&id="+id,true);
        xmlhttp.send();
...
function AWB_Details(CourierVendor,CourierService,Airwaybill_Number,AWB_Date,COD,PackageCount,Shipper_Name,Shipper_Comp,Shipper_Address,Receiver_Name,Receiver_Comp,Receiver_Address,sender_city,sender_state,sender_pincode,sender_phone,sender_vendorid){
...
function CancelAWB(RUID,AWB_Date,CourierVendor,Airwaybill_Number){
    
    if (confirm("Are you sure you want to delete Receive Details?") == true) {
        ...
        xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                showalert('Courier receive details deleted successfuly!','alert-success');
            }
        };
        xmlhttp.open("GET","CancelAWB.php?Airwaybill_Number="+Airwaybill_Number+"&RUID="+RUID+"&cancel_type=Receive",true);
        xmlhttp.send();
...
function CourierVendorList(){
    ...
    xmlhttp.open("GET","vendoraction.php?action=ReceiveCourierList&id="+id,true);
    xmlhttp.send();
